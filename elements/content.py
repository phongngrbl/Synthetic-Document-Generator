    ...
    def generate(self, size):
        ...
        for layout in layouts:
            ...
            for bbox, align in layout:
                ...
                for text_tmp, text_layer_tmp in zip(text, text_layer):
                    text_tmp_left = text_layer_tmp.left
                    text_tmp_right = text_layer_tmp.right
                    tmp_center_x = text_layer_tmp.centerx
                    tmp_center_y = text_layer_tmp.centery
                    self.reader.prev()

                    if text_layer_tmp is None:
                        continue

                    text_layer_tmp.center = (tmp_center_x + x + w / 2,tmp_center_y + y + h / 2)
                    if align == "left":
                        text_layer_tmp.left = x + text_tmp_left
                    if align == "right":
                        text_layer_tmp.right = x + text_tmp_right

                    self.textbox_color.apply([text_layer_tmp])
                    text_layers.append(text_layer_tmp)
                    texts.append(text_tmp)
